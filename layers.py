...
class BilinearDecoder(Layer):
    ...
    def _call(self, inputs):
        ...
        inputs = tf.matmul(inputs, self.vars['weights'])   # 加入的参数
        x = tf.matmul(inputs, x)
        x = tf.reshape(x, [-1])
        outputs = self.act(x)
        return outputs
class Fei_InnerProductDecoder(Layer):
    """Decoder model layer for link prediction."""

    def __init__(self, input_dim, dropout=0., act=tf.nn.sigmoid, **kwargs):
        super(Fei_InnerProductDecoder, self).__init__(**kwargs)
    ...
    def _call(self, inputs):
        ...
        outputs = tf.stack(outputs, axis=2)  # (n,n,10)

        #################
        # decoder
        outputs = tf.reshape(outputs, [-1, 10])
        logits_output = tf.nn.softmax(outputs)  # (n*n,10)
        ...
        M_hat = tf.reshape(M_hat, [n, n])  # (n,n) score matrix which between 1 and 10
        #M_hat = tf.reshape(M_hat, [-1])
        ################
